...
const MonthlyTable: React.FC<MonthlyTableProps> = ({ data, accentColor }) => {
  return (
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {data.map((row, index) => (
              <tr 
                key={index} 
                className={`group transition-all duration-300 ${row.isPeak ? 'bg-white/[0.02]' : 'hover:bg-white/[0.01]'}`}
              >
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-bold text-slate-300 group-hover:text-white transition-colors">{row.month}</span>
                    {row.isPeak && (
                ...
                <td className="p-4 text-sm font-mono font-medium text-slate-200">
